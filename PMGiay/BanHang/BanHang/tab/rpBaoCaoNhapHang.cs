...
{
    public partial class rpBaoCaoNhapHang : UserControl
    {
        AutoCompleteStringCollection namesCollection = new AutoCompleteStringCollection();
        public rpBaoCaoNhapHang()
        {
            InitializeComponent();
        ...
        public void ReportLoad()
        {
            ...
            string sql = "SELECT ROW_NUMBER() OVER (ORDER BY NGAYNHAP  ASC) [MODIFYBY], MANHAP, CONVERT(VARCHAR(20),NGAYNHAP,103) AS NGAYNHAP , CHUHANG, HOTEN, DIACHI, SODT, SOTK, NGANHANG, CHIETKHAU, MAHANG, TENHANG, SIZE, SLNHAP, DONGIA, THANHTIEN, GHICHU, SLBAN, THANHTOAN,  CONVERT(VARCHAR(20),NGAYTHANHTOAN,103) AS NGAYTHANHTOAN , GHICHUTT, LOINHUAN, CREATEBY, CREATEDATE,  MODIFYDATE";
            sql += " FROM NHAP_HANG ";
            sql += " WHERE  CONVERT(DATETIME,NGAYNHAP,103) BETWEEN CONVERT(DATETIME,'" + tungay + "',103) AND CONVERT(DATETIME,'" + denngay + "',103) ";
            ...
            this.reportViewer1.LocalReport.DataSources.Add(new Microsoft.Reporting.WinForms.ReportDataSource("dsNhapHang", bang));
